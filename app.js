...
let speechUnlocked=false,availableVoices=[],audioCtx=null,speechQueue=[],speechBusy=false,speechWatchdog=null,voiceUnlockDismissed=false;
...
function markSpeechUnlockedSession(){try{sessionStorage.setItem('naviVoiceUnlockedThisSession','1')}catch(e){}}
function isSpeechUnlockedSession(){try{return sessionStorage.getItem('naviVoiceUnlockedThisSession')==='1'}catch(e){return false}}
...
node["amenity"~"fuel|parking|restaurant|cafe|hospital|pharmacy"](${bbox});
way["amenity"~"fuel|parking|restaurant|cafe|hospital|pharmacy"](${bbox});
node["highway"~"speed_camera|construction|traffic_signals"](${bbox});
way["highway"~"speed_camera|construction|traffic_signals"](${bbox});
...
function normalizeOverpassElement(el){
    ...
    let key='';
    if(tags.brand&&osmTypeMap['brand_'+tags.brand])key='brand_'+tags.brand;
    else if(tags.amenity)key='amenity_'+tags.amenity;
    else if(tags.highway)key='highway_'+tags.highway;
    else if(tags.barrier)key='barrier_'+tags.barrier;
    else if(tags.enforcement==='maxspeed')key='highway_speed_camera';
    const meta=osmTypeMap[key];
    if(!meta)return null;
    const isSpeedCamera=meta.isSpeedCamera||tags.highway==='speed_camera'||tags.enforcement==='maxspeed';
    return {
        lat,lng,
        name:tags.name||tags.brand||(isSpeedCamera?'Fotoradar':'POI'),
        limit:isSpeedCamera?parseSpeedLimit(tags.maxspeed):undefined,
        icon:meta.icon,
        type:meta.type,
        style:meta.style,
        source:'overpass'
...
function uniquePoiList(items){
    const seen=new Set();
    return items.filter(item=>{
        const key=`${item.type}:${Math.round(item.lat*10000)}:${Math.round(item.lng*10000)}:${item.name||''}`;
        if(seen.has(key))return false;
        seen.add(key);
        return true;
    });
}
async function fetchOnlineRoutePOIs(minLat,maxLat,minLng,maxLng){
    if(appState.offlineNavigation&&!navigator.onLine)return[];
    const body='data='+encodeURIComponent(buildOverpassQuery(minLat,maxLat,minLng,maxLng));
    const response=await fetch(CONFIG.overpassUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body});
...
async function loadOSMPOIs(minLat,maxLat,minLng,maxLng){
    ...
    try{
        ...
        if(seq!==appState.poiLoadSeq)return;
...
function nextInstructionDistance(progress){return core.distanceToNextInstructionKm(progress,appState.routeInstructions,appState.routeCumulativeDists,appState.instructionIndex)}
function currentInstructionDoneKm(){
    const inst=appState.routeInstructions[appState.instructionIndex];
    if(!inst||typeof inst.index!=='number')return null;
    return appState.routeCumulativeDists[inst.index]||0;
}
function getNavigationCamera(progress,pos){
    const nextDist=nextInstructionDistance(progress);
    let zooming=false;
    if(appState.junctionFocusDoneKm!==null){
        zooming=(progress.doneKm||0)<=appState.junctionFocusDoneKm+CONFIG.junctionKeepAfterKm;
        if(!zooming)appState.junctionFocusDoneKm=null;
    }
    if(!zooming&&nextDist<=CONFIG.junctionZoomDistanceKm){
        const done=currentInstructionDoneKm();
        if(done!==null){
            appState.junctionFocusDoneKm=done;
            zooming=true;
        }
    }
    return {mode:zooming?'junction':'overview',center:pos,zoom:zooming?CONFIG.junctionZoom:cameraZoom(),nextDist};
}
function fitActiveRouteOverview(animate=false){
    if(!appState.routeCoords.length)return;
    const bounds=L.latLngBounds(appState.routeCoords.map(c=>[c.lat,c.lng]));
    if(appState.userPos)bounds.extend(appState.userPos);
    if(appState.destination)bounds.extend(appState.destination);
    map.invalidateSize({pan:false});
    map.fitBounds(bounds,{paddingTopLeft:[80,110],paddingBottomRight:[90,120],animate});
}
function smoothSetView(pos,progress=appState.routeProgress,opts={}){
    ...
    appState.navCameraMode=camera.mode;
...
function startSmoothNavigation(progress){
    if(!appState.navigationActive||!appState.routeCoords.length||!progress||!progress.snapped)return;
    const target=progress.doneKm||0;
    let from=typeof appState.smoothNav.lastRenderDoneKm==='number'?appState.smoothNav.lastRenderDoneKm:target;
    if(Math.abs(target-from)>0.8)from=target;
    if(target<from)from=target;
    const distKm=Math.max(0,target-from);
    const speedKmh=Math.max(appState.currentSpeed||0,CONFIG.navAnimSpeedFloorKmh);
    const durationMs=Math.max(CONFIG.navAnimMinMs,Math.min(CONFIG.navAnimMaxMs,(distKm/speedKmh)*3600000||CONFIG.navAnimMinMs));
    cancelSmoothNavigation();
    appState.smoothNav.fromDoneKm=from;
    appState.smoothNav.toDoneKm=target;
    appState.smoothNav.startTime=performance.now();
    appState.smoothNav.durationMs=durationMs;
    if(distKm<0.002){renderSmoothNavigation(target);return}
    const step=ts=>{
        const t=Math.min(1,(ts-appState.smoothNav.startTime)/appState.smoothNav.durationMs);
        const done=appState.smoothNav.fromDoneKm+(appState.smoothNav.toDoneKm-appState.smoothNav.fromDoneKm)*t;
        renderSmoothNavigation(done);
        if(t<1)appState.smoothNav.raf=requestAnimationFrame(step);
        else appState.smoothNav.raf=null;
    };
    appState.smoothNav.raf=requestAnimationFrame(step);
}
...
function processGpsPosition(pos){
    if(appState.simulation.active&&!pos.synthetic)return;
...
function pollGps(){if(appState.simulation.active||!navigator.geolocation)return;appState.lastGpsPoll=Date.now();navigator.geolocation.getCurrentPosition(processGpsPosition,handleGpsError,CONFIG.gpsOptions)}
function startGpsTracking(){
    if(!navigator.geolocation)return handleGpsError({message:'Geolocation unavailable'});
    if(appState.gpsWatchId===null&&navigator.geolocation.watchPosition){
        appState.gpsWatchId=navigator.geolocation.watchPosition(processGpsPosition,handleGpsError,CONFIG.gpsOptions);
    }
    pollGps();
}
startGpsTracking();
setInterval(()=>{if(!appState.simulation.active&&Date.now()-appState.gpsQuality.lastFix>CONFIG.gpsIntervalMs)pollGps()},CONFIG.gpsIntervalMs);
...
function startSimulation(){
    ...
    cancelSmoothNavigation();
    appState.smoothNav.lastRenderDoneKm=appState.routeProgress.doneKm||0;
    ...
    appState.simulation.timer=setInterval(()=>{
        ...
        processGpsPosition({synthetic:true,coords:{latitude:p.lat,longitude:p.lng,heading:p.bearing,accuracy:5,speed:appState.simulation.speedKmh/3.6}});
...
function selectRoute(routeIndex, routes = appState.alternativeRoutes) {
    ...
    appState.navCameraMode='idle';
    appState.junctionFocusDoneKm=null;
    cancelSmoothNavigation();
